...
function SetModulesPositionX()
	...
	for _, module in ipairs(modules) do
		...
		if module.active and (module.name ~= "share" or not hideShareIcons) then
			if
				(module.name == "cpuping" and isSinglePlayer)
				or (module.name == "resources" and isSingle)
				or (module.name == "income" and playerScale < 0.7)
			then
			else
		...
		if #teams > 1 then
			for _, teamID in ipairs(teams) do
				if teamID ~= gaiaTeamID then
					...
					if history then
						...
						scores[allyTeamID][#scores[allyTeamID] + 1] = {
							teamID = teamID,
							score = mathFloor(
								(history.metalUsed + history.energyUsed / 60 + history.damageDealt) / 1000
							),
						}
					end
				end
			...
			if scores[allyTeamID] and (mySpecStatus or not hoverPlayerlist or allyTeamID ~= myAllyTeamID) then
				...
				for i, params in ipairs(scores[allyTeamID]) do
					...
					if
						not teamRanking[allyTeamID]
						or not teamRanking[allyTeamID][i]
						or ranking[i] ~= teamRanking[allyTeamID][i]
					then
						rankingChanged = true
					end
				end
				...
				if teamLeaderID and teamLeaderID >= 0 then
					...
					if pName and not pSpec and pTeam == i then
						lastKnownTeamNames[i] = (WG.playernames and WG.playernames.getPlayername)
								and WG.playernames.getPlayername(teamLeaderID)
							or pName
					end
				end
			end
...
function CreatePlayer(playerID)
	local tname, _, tspec, tteam, tallyteam, tping, tcpu, tcountry, trank, _, accountInfo, desynced =
		sp.GetPlayerInfo(playerID)
	local accountID = nil
	...
	-- resources
	local energy, energyStorage, energyIncome, energyShare, metal, metalStorage, metalIncome, metalShare =
		0, 1, 0, 0, 1, 0, 0, 0
...
function SortAllyTeams(vOffset)
	...
	if not allyTeamRanking or not enemyListShow then
		local showOwnAlly = not mySpecStatus
			or (not hideDeadAllyTeams or (aliveAllyTeams[myAllyTeamID] and populatedAllyTeams[myAllyTeamID]))
		if showOwnAlly then
	...
	if numberOfEnemies > 0 then
		...
		if enemyListShow or not ownAllyTeamDrawn then
			...
			for _, allyTeamID in ipairs(allyTeamList) do
				if
					(allyTeamRanking or allyTeamID ~= myAllyTeamID)
					and (not hideDeadAllyTeams or aliveAllyTeams[allyTeamID])
				then
					if firstenemy then
						firstenemy = false
					else
	...
	for _, i in ipairs(drawList) do
		if i > -1 then -- and i < specOffset
			...
			if
				...
				and not p.spec
				and not p.dead
				and p.name ~= absentName
				and IsOnRect(x, y, widgetPosX, posY, widgetPosX + widgetWidth, posY + (playerOffset * playerScale))
			then
				if mySpecStatus or (myAllyTeamID == p.allyteam and not sliderPosition) then
					UiSelectHighlight(
						widgetPosX,
						posY,
						widgetPosX + widgetPosX + 2 + 4,
						posY + (playerOffset * playerScale),
						nil,
						b and 0.28 or 0.14
					)
				end
...
function CreateLists(onlyMainList, onlyMainList2, onlyMainList3)
	...
	if updateMainLists then
		updateMainLists = {
			onlyMainList and onlyMainList or updateMainLists[1],
			onlyMainList2 and onlyMainList2 or updateMainLists[2],
			onlyMainList3 and onlyMainList3 or updateMainLists[3],
		}
	else
...
function CreateBackground()
	...
	if forceMainListRefresh or not mainListBgTex or (WG.guishader and not BackgroundGuishader) then
		if WG.guishader then
			...
			BackgroundGuishader = gl_CreateList(function()
				RectRound(
					...
					absTop,
					elementCorner,
					mathMin(paddingLeft, paddingTop),
					mathMin(paddingTop, paddingRight),
					mathMin(paddingRight, paddingBottom),
					mathMin(paddingBottom, paddingLeft)
				)
			end)
		...
		local width, height =
			mathFloor(apiAbsPosition[4] - apiAbsPosition[2]), mathFloor(apiAbsPosition[1] - apiAbsPosition[3])
...
function drawMainList()
	...
	for i, drawObject in ipairs(drawList) do
		if drawObject == -5 then
			...
			DrawLabel(
				" " .. BAR.I18N("ui.playersList.spectators", { amount = specAmount }),
				drawListOffset[i],
				specListShow
			)
			if spGetGameFrame() <= 0 then
		...
		elseif drawObject == -3 then
			if numberOfEnemies > 0 then
				...
				if allyTeamRanking and enemyListShow then
					...
					DrawLabel(
						" " .. BAR.I18N("ui.playersList.enemies", { amount = enemyAmount }),
						drawListOffset[i],
						true
					)
				end
	...
	if onlyMainList then
		...
		if mainListTex then
			gl.R2tHelper.RenderToTexture(mainListTex, function()
				...
				gl.Translate(
					-apiAbsPosition[2] - (backgroundMargin * 0.25 * scaleMult),
					-apiAbsPosition[3] - (backgroundMargin * 0.25 * scaleMult),
					0
				)
				drawMainList()
			end, true)
...
function DrawPlayer(playerID, leader, vOffset, mouseX, mouseY, onlyMainList, onlyMainList2, onlyMainList3)
	...
	if not spec then
		...
		if
			onlyMainList2
			and m_alliance.active
			and drawAllyButton
			and not mySpecStatus
			and not dead
			and team ~= myTeamID
		then
		...
		if
			(onlyMainList2 or onlyMainList3)
			and not isSingle
			and (m_resources.active or m_income.active)
			and aliveAllyTeams[allyteam] ~= nil
			and p.energy ~= nil
		then
				...
				if es and es > 0 then
					if onlyMainList3 and m_resources.active and e and (not dead or (e > 0 or m > 0)) then
						DrawResources(
							...
							dead,
							(
								absoluteResbarValues
								and (allyTeamMaxStorage[allyteam] and allyTeamMaxStorage[allyteam][1])
							),
							(
								absoluteResbarValues
								and (allyTeamMaxStorage[allyteam] and allyTeamMaxStorage[allyteam][2])
							)
						)
						if tipY then
		...
		if cpuLvl ~= nil then
			...
			if tipY then
				PingCpuTip(
					...
					ping,
					cpu,
					lastFpsData[playerID],
					lastGpuMemData[playerID],
					lastLuaMemData[playerID],
					lastSystemData[playerID],
					name,
					team,
					spec,
					lastApmData[team]
...
function DrawTakeSignal(posY)
	if blink then
		...
		DrawRect(
			widgetPosX - (57 * playerScale),
			posY - (14 * playerScale),
			widgetPosX - (12 * playerScale),
			posY + (31 * playerScale)
		)
		gl_Color(1, 1, 1, 1)
	end
...
function DrawChatButton(posY)
	...
	DrawRect(
		m_chat.posX + widgetPosX + (1 * playerScale),
		posY,
		m_chat.posX + widgetPosX + (17 * playerScale),
		posY + (16 * playerScale)
	)
end
...
function DrawResources(
	...
	posY,
	dead,
	maxAllyTeamEnergyStorage,
	maxAllyTeamMetalStorage
)
	...
	DrawRect(
		m_resources.posX + widgetPosX + paddingLeft,
		posY + y1Offset,
		m_resources.posX + widgetPosX + paddingLeft + (barWidth * (metalStorage / maxStorage)),
		posY + y2Offset
	)
	gl_Color(1, 1, 1, 1)
	...
	DrawRect(
		m_resources.posX + widgetPosX + paddingLeft,
		posY + y1Offset,
		m_resources.posX + widgetPosX + paddingLeft + ((barWidth / maxStorage) * metal),
		posY + y2Offset
	)
	...
	if playerScale >= 0.9 and (barWidth / maxStorage) * metal > 0.8 then
		...
		DrawRect(
			m_resources.posX + widgetPosX + paddingLeft,
			posY + y1Offset + glowsize,
			m_resources.posX + widgetPosX + paddingLeft + ((barWidth / maxStorage) * metal),
			posY + y2Offset - glowsize
		)

		gl_Texture(pics.barGlowEdgePic)
		DrawRect(
			m_resources.posX + widgetPosX + paddingLeft - (glowsize * 1.8),
			posY + y1Offset + glowsize,
			m_resources.posX + widgetPosX + paddingLeft,
			posY + y2Offset - glowsize
		)
		DrawRect(
			m_resources.posX + widgetPosX + paddingLeft + ((barWidth / maxStorage) * metal) + (glowsize * 1.8),
			posY + y1Offset + glowsize,
			m_resources.posX + widgetPosX + paddingLeft + ((barWidth / maxStorage) * metal),
			posY + y2Offset - glowsize
		)
	...
	if metalShare < 0.99 then -- default = 0.99
		...
		DrawRect(
			m_resources.posX
				+ widgetPosX
				+ paddingLeft
				+ ((barWidth * (metalStorage / maxStorage)) * metalShare)
				- 0.75
				- bordersize,
	...
	DrawRect(
		m_resources.posX + widgetPosX + paddingLeft,
		posY + y1Offset,
		m_resources.posX + widgetPosX + paddingLeft + (barWidth * (energyStorage / maxStorage)),
		posY + y2Offset
	)
	gl_Color(1, 1, 0, 1)
	gl_Texture(pics.resbarPic)
	DrawRect(
		m_resources.posX + widgetPosX + paddingLeft,
		posY + y1Offset,
		m_resources.posX + widgetPosX + paddingLeft + ((barWidth / maxStorage) * energy),
		posY + y2Offset
	)
	...
	if playerScale >= 0.9 and (barWidth / maxStorage) * energy > 0.8 then
		...
		DrawRect(
			m_resources.posX + widgetPosX + paddingLeft + ((barWidth / maxStorage) * energy) + (glowsize * 1.8),
			posY + y1Offset + glowsize,
			m_resources.posX + widgetPosX + paddingLeft + ((barWidth / maxStorage) * energy),
			posY + y2Offset - glowsize
		)
	...
	if energyConversion and energyConversion ~= 0.75 and not dead then -- default = 0.75
		...
		DrawRect(
			m_resources.posX
				+ widgetPosX
				+ paddingLeft
				+ ((barWidth * (energyStorage / maxStorage)) * energyConversion)
				- 0.75
				- bordersize,
			posY + y1Offset + 0.55 + bordersize,
			m_resources.posX
	...
	if energyShare < 0.94 or energyShare > 0.96 then -- default = 0.94999999
		...
		DrawRect(
			m_resources.posX
				+ widgetPosX
				+ paddingLeft
				+ ((barWidth * (energyStorage / maxStorage)) * energyShare)
				- 0.75
				- bordersize,
			posY + y1Offset + 1.1,
			m_resources.posX
...
function DrawSidePic(team, playerID, posY, leader, dark, ai)
	...
	if gameStarted then
		...
		DrawRect(
			m_side.posX + widgetPosX + (2 * playerScale),
			posY + (1 * playerScale),
			m_side.posX + widgetPosX + (16 * playerScale),
			posY + (15 * playerScale)
		) -- draws side image
		gl_Texture(false)
	else
...
function DrawRankImage(rankImage, posY)
	...
	DrawRect(
		m_rank.posX + widgetPosX + (3 * playerScale),
		posY + (8 * playerScale) - (7.5 * playerScale),
		m_rank.posX + widgetPosX + (17 * playerScale),
		posY + (8 * playerScale) + (7.5 * playerScale)
	)
end
...
function DrawAlly(posY, team)
	...
	DrawRect(
		m_alliance.posX + widgetPosX + (3 * playerScale),
		posY + (1 * playerScale),
		m_alliance.posX + widgetPosX + (playerOffset * playerScale),
		posY + (15 * playerScale)
	)
end
...
function DrawCountry(country, posY)
	...
	if cached then
		...
		gl_Color(1, 1, 1, 1)
		DrawRect(
			m_country.posX + widgetPosX + (3 * playerScale),
			posY + (8 * playerScale) - ((images.flagHeight / 2) * playerScale),
			m_country.posX + widgetPosX + (17 * playerScale),
			posY + (8 * playerScale) + ((images.flagHeight / 2) * playerScale)
		)
	end
end
...
function DrawDot(posY)
	...
	DrawRect(
		m_indent.posX + widgetPosX - 1,
		posY + (3 * playerScale),
		m_indent.posX + widgetPosX + (7 * playerScale),
		posY + (11 * playerScale)
	)
end
...
function DrawCamera(posY, active)
	...
	DrawRect(
		m_indent.posX + widgetPosX - (1.5 * playerScale),
		posY + (2 * playerScale),
		m_indent.posX + widgetPosX + (9 * playerScale),
		posY + (12.4 * playerScale)
	)
end
...
function DrawState(playerID, posX, posY)
	...
	local ready = (playerReadyState[playerID] == 1)
...
function DrawAlliances(alliances, posY)
	...
	for i, allyPlayerID in pairs(alliances) do
		...
		if ap ~= nil and ap.red ~= nil then
			...
			RectRound(
				posX + (width * (i - 1)) + padding,
				posY - (3 * playerScale) + padding,
				posX + (width * i) - padding,
				posY + (19 * playerScale) - padding,
				(2 * playerScale)
			)
			drawn = true
		end
...
function DrawName(name, nameIsAlias, team, posY, dark, playerID, accountID, desynced)
	...
	if not gameStarted then
		if playerID >= specOffset then
			willSub = (Spring.GetGameRulesParam("Player" .. (playerID - specOffset) .. "willSub") == 1) and " (sub)"
				or "" --pID-specOffset because apl uses dummy playerIDs for absent players
		else
	...
	if desynced then
		...
		font2:Print(
			BAR.I18N("ui.playersList.desynced"),
			m_name.posX + widgetPosX + 5 + xPadding + (font2:GetTextWidth(nameText) * 14 * pScale),
			posY + (5.7 * playerScale),
			8 * pScale,
			"o"
		)
	elseif pDraw and not pDraw.dead and pDraw.incomeMultiplier and pDraw.incomeMultiplier ~= 1 then
		...
		if pDraw.incomeMultiplier > 1 then
			...
			font2:Print(
				"+" .. mathFloor((pDraw.incomeMultiplier - 1 + 0.005) * 100) .. "%",
				m_name.posX + widgetPosX + 5 + xPadding + (font2:GetTextWidth(nameText) * 14 * pScale),
				posY + (5.7 * playerScale),
				8 * pScale,
				"o"
...
function DrawSmallName(name, nameIsAlias, team, posY, dark, playerID, accountID, alpha)
	...
	font2:Print(
		nameText,
		m_name.posX + textindent + widgetPosX + 3,
		posY + (4 * specScale),
		(10 * specScale * fontScaleSpec),
		"n"
	)
	font2:End()
...
function DrawAllyID(allyID, posY, dark, dead)
	...
	font:Print(
		spacer .. allyID,
		m_allyID.posX + widgetPosX + (4.5 * playerScale),
		posY + (5.3 * playerScale),
		fontsize,
		"on"
	)
	font:End()
...
function DrawPlayerID(playerID, posY, dark, spec)
	...
	font:Print(
		spacer .. playerID,
		m_playerID.posX + widgetPosX + (4.5 * usedScale),
		posY + (5.3 * usedScale),
		fontsize,
		"on"
	)
...
function DrawID(teamID, posY, dark, dead)
	...
	font:Print(
		spacer .. teamID,
		m_ID.posX + widgetPosX + (4.5 * playerScale),
		posY + (5.3 * playerScale),
		fontsize,
		"on"
	)
...
function DrawPingCpu(pingLvl, cpuLvl, posY, spec, cpu, fps)
	...
	if spec then
		...
		DrawRect(
			m_cpuping.posX + widgetPosX + (12 * specScale),
			posY + (1 * specScale),
			m_cpuping.posX + widgetPosX + (21 * specScale),
			posY + (14 * specScale)
		)
	else
		gl_Color(pingLevelData[pingLvl].r, pingLevelData[pingLvl].g, pingLevelData[pingLvl].b)
		DrawRect(
			m_cpuping.posX + widgetPosX + (12 * playerScale),
			posY + (1 * playerScale),
			m_cpuping.posX + widgetPosX + (24 * playerScale),
			posY + (15 * playerScale)
		)
	end
	...
	if fps ~= nil then
		...
		if spec then
			...
			font:Print(
				fps,
				m_cpuping.posX + widgetPosX + (11 * specScale),
				posY + (5.3 * playerScale),
				9 * specScale * fontScale,
				"ro"
			)
		else
			...
			font:Print(
				fps,
				m_cpuping.posX + widgetPosX + (11 * playerScale),
				posY + (5.3 * playerScale),
				9.5 * playerScale * fontScale,
				"ro"
			)
		end
	else
		...
		if spec then
			...
			DrawRect(
				m_cpuping.posX + widgetPosX + (2 * specScale),
				posY + (1 * specScale),
				m_cpuping.posX + widgetPosX + (13 * specScale),
				posY + (14 * specScale * fontScale)
			)
		else
			gl_Color(pingLevelData[cpuLvl].r, pingLevelData[cpuLvl].g, pingLevelData[cpuLvl].b)
			DrawRect(
				m_cpuping.posX + widgetPosX + (1 * playerScale),
				posY + (1 * playerScale),
				m_cpuping.posX + widgetPosX + (14 * playerScale),
				posY + (15 * playerScale * fontScale)
			)
		end
		gl_Color(1, 1, 1, 1)
	end
...
function DrawPoint(posY, pointtime)
	...
	DrawRect(
		widgetPosX - (33 * playerScale),
		posY - (1 * playerScale),
		widgetPosX - (17 * playerScale),
		posY + (15 * playerScale)
	)
	gl_Color(1, 1, 1, 1)
end
...
function DrawPencil(posY, time)
	...
	DrawRect(
		m_indent.posX + widgetPosX - 3.5,
		posY + (3 * playerScale),
		m_indent.posX + widgetPosX - 1.5 + (8 * playerScale),
		posY + (14 * playerScale)
	)
	gl_Color(1, 1, 1, 1)
end
	...
	then
		...
		if pTip.history and (nameIsAlias or #pTip.history > 1) then
			...
			if c > 0 then
				tipText = text
				tipTextTime = osClock()
				tipTextTitle = (originalColourNames[playerID] and colourNames(pTip.team) or "\255\255\255\255")
					.. pTip.name
			end
		end
	end
...
function AllyTip(mouseX, playerID)
	if
		mouseX >= widgetPosX + (m_alliance.posX + (1 * playerScale)) * widgetScale
...
function ResourcesTip(mouseX, energy, energyStorage, energyIncome, metal, metalStorage, metalIncome, name, teamID)
	if
		mouseX >= widgetPosX + (m_resources.posX + (1 * playerScale)) * widgetScale
		and mouseX <= widgetPosX + (m_resources.posX + (m_resources.width * playerScale)) * widgetScale
	then
		if energy > 1000 then
...
function IncomeTip(mouseX, energyIncome, metalIncome, name, teamID)
	if
		mouseX >= widgetPosX + (m_income.posX + (1 * playerScale)) * widgetScale
		and mouseX <= widgetPosX + (m_income.posX + m_resources.width) * widgetScale
	then
		if energyIncome == nil then
			energyIncome = 0
			metalIncome = 0
...
function PingCpuTip(mouseX, pingLvl, cpuLvl, fps, gpumem, luamem, system, name, teamID, spec, apm)
	if
		mouseX >= widgetPosX + (m_cpuping.posX + (13 * playerScale)) * widgetScale
	...
	then
		...
		tipText = BAR.I18N(
			"ui.playersList.commandDelay",
			{ labelColor = "\255\190\190\190", delayColor = "\255\255\255\255", delay = pingLvl }
		)
		tipTextTitle = (spec and "\255\240\240\240" or colourNames(teamID)) .. name
		tipTextTime = osClock()
	elseif
		mouseX >= widgetPosX + (m_cpuping.posX + (1 * playerScale)) * widgetScale
	...
	ShareSlider = gl_CreateList(function()
		...
		if sliderPosition then
			...
			if energyPlayer ~= nil then
				...
				DrawRect(
					m_share.posX + widgetPosX + (16 * playerScale),
					posY - (3 * playerScale),
					m_share.posX + widgetPosX + (34 * playerScale),
					posY + shareSliderHeight + (18 * playerScale)
				)
				gl_Texture(pics.energyPic)
				DrawRect(
					m_share.posX + widgetPosX + (17 * playerScale),
					posY + sliderPosition,
					m_share.posX + widgetPosX + (33 * playerScale),
					posY + (16 * playerScale) + sliderPosition
				)
				gl_Texture(false)
				gl_Color(0.45, 0.45, 0.45, 1)
				RectRound(
					mathFloor(m_share.posX + widgetPosX - (28 * playerScale)),
					mathFloor(posY - 1 + sliderPosition),
					mathFloor(m_share.posX + widgetPosX + (19 * playerScale)),
					mathFloor(posY + (17 * playerScale) + sliderPosition),
					2.5 * playerScale
				)
				font:Print(
			...
			elseif metalPlayer ~= nil then
				...
				gl_Texture(pics.barPic)
				DrawRect(
					m_share.posX + widgetPosX + (32 * playerScale),
					posY - 3,
					m_share.posX + widgetPosX + (50 * playerScale),
					posY + shareSliderHeight + (18 * playerScale)
				)
				gl_Texture(pics.metalPic)
				DrawRect(
				...
				gl_Color(0.45, 0.45, 0.45, 1)
				RectRound(
					mathFloor(m_share.posX + widgetPosX - (12 * playerScale)),
					mathFloor(posY - 1 + sliderPosition),
					mathFloor(m_share.posX + widgetPosX + (35 * playerScale)),
					mathFloor(posY + (17 * playerScale) + sliderPosition),
					2.5 * playerScale
				)
				font:Print(
...
function widget:MousePress(x, y, button)
	...
	if button == 1 then
		...
		if
			numberOfEnemies > 0 and IsOnRect(x, y, widgetPosX + 2, posY + 2, widgetPosX + widgetWidth - 2, posY + 20)
		then
		...
		for _, i in ipairs(drawList) do
			...
			if mySpecStatus then
							...
							if clickedPlayer.pointTime ~= nil then
								...
								then
									...
									Spring.SetCameraTarget(
										clickedPlayer.pointX,
										clickedPlayer.pointY,
										clickedPlayer.pointZ,
										1
									)
									return true
								end
				...
				if i > -1 then -- and i < specOffset
					if
						m_name.active
						and clickedPlayer.name ~= absentName
						and IsOnRect(
							x,
							y,
							widgetPosX,
							posY,
							widgetPosX + widgetWidth,
							posY + (playerOffset * playerScale)
						)
					then
						if ctrl and i < specOffset then
							sp.SendCommands(
								"toggleignore "
									.. (clickedPlayer.accountID and clickedPlayer.accountID or clickedPlayer.name)
							)
							return true
						...
						if
							i < specOffset
							and (mySpecStatus or player[i].allyteam == myAllyTeamID)
							and clickTime - prevClickTime < dblclickPeriod
							and clickedPlayer == prevClickedPlayer
						then
					...
					if i > -1 and i < specOffset then
						...
						if m_chat.active then
							if
								IsOnRect(
									x,
									y,
									m_chat.posX + widgetPosX + 1,
									posY,
									m_chat.posX + widgetPosX + 17,
									posY + (playerOffset * playerScale)
								)
							then
						...
						then
							if
								IsOnRect(
									x,
									y,
									m_alliance.posX + widgetPosX + 1,
									posY,
									m_alliance.posX + widgetPosX + m_alliance.width,
									posY + (playerOffset * playerScale)
								)
							then
						...
						if
							...
							and IsOnRect(
								x,
								y,
								m_name.posX + widgetPosX + 1,
								posY,
								m_name.posX + widgetPosX + m_name.width,
								posY + 12
							)
						then
							if ctrl then
							...
							if
								(mySpecStatus or player[i].allyteam == myAllyTeamID)
								and clickTime - prevClickTime < dblclickPeriod
								and clickedPlayer == prevClickedPlayer
							then
...
function widget:MouseMove(x, y, dx, dy, button)
	if energyPlayer ~= nil or metalPlayer ~= nil then
		...
		if
			playSounds
			and (lastSliderSound == nil or osClock() - lastSliderSound > 0.05)
			and shareAmount ~= prevAmountEM
		then
...
function widget:SetConfigData(data)
	...
	if
		data.lastSystemData ~= nil
		and data.gameFrame ~= nil
		and data.gameFrame <= spGetGameFrame()
		and data.gameFrame > spGetGameFrame() - 300
	then
...
function CheckPlayersChange()
	...
	for i = 0, specOffset - 1 do
		local name, active, spec, teamID, allyTeamID, pingTime, cpuUsage, _, rank, _, _, desynced =
			sp.GetPlayerInfo(i, false)
...
function widget:Update(delta)
	...
	if
		m_take.active
		and mySpecStatus == false
		and mx >= widgetPosX - 57 * widgetScale
		and mx <= widgetPosX - 1 * widgetScale
	then
		for i, drawObject in ipairs(drawList) do
			if drawObject >= 0 then
				local p = player[drawObject]
	...
	if curFrame >= 30 + tookFrame then
		if lastTakeMsg + 120 < tookFrame and reportTake then
			...
			if detailedToSay then
				Spring.SendLuaRulesMsg(
					"msg:ui.playersList.chat.takeTeam:name="
						.. tookTeamName
						.. ":units="
						.. mathFloor(afterU)
						.. ":energy="
	...
	if forceMainListRefresh then
		...
		local updateMainList3 = (
			(m_resources.active or m_income.active) and timeFastCounter > updateFastRate * updateFastRateMult
		)
